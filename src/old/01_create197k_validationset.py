...
human_dataset = get_dataset('human', 'valid', "/users/cn/lsantus/data/FED/basenji").batch(1).repeat()

print("-----------------------------------------------------------------------------")
print("human dataset loaded")
print(type(human_dataset))
...
for i, batch in tqdm(enumerate(human_dataset)):
    ...
    batch_197k["sequence"] = sequence_197k[np.newaxis]
